...
// Generate unsigned upload preset URL (client-side uploads without backend)
export const getCloudinaryUploadPreset = () => {
  if (!process.env.CLOUDINARY_UPLOAD_PRESET) {
    throw new Error('CLOUDINARY_UPLOAD_PRESET environment variable is required');
  }
  return process.env.CLOUDINARY_UPLOAD_PRESET;
};

// Generate signed upload signature (for secure server-side uploads)
export const generateCloudinarySignature = async (params: Record<string, any>) => {
  try {
