...
import { getMediaComments, getMediaList, replyToComment } from "@/lib/meta";
import { getYouTubeComments, replyToYouTubeComment } from "@/lib/youtube";
...
export default function CommentsPage() {
    ...
    try {
        ...
        try {
          ...
          for (const item of media.slice(0, 10)) {
            try {
              ...
              console.error(
                `[v0] Failed to load comments for Instagram media ${item.id}`
              );
            }
          }
        } catch (err) {
          console.error(
            `[v0] Failed to load Instagram media for ${igAccount.username}`
          );
        }
      }
      ...
      for (const ytAccount of ytAccounts) {
        try {
          ...
          if (channelResponse.ok) {
            ...
            for (const video of channelData.items || []) {
              try {
                ...
                console.error(
                  `[v0] Failed to load comments for YouTube video ${video.id.videoId}`
                );
              }
            }
          }
          ...
          console.error(
            `[v0] Failed to load YouTube videos for ${ytAccount.name}`
          );
        }
      }
  ...
  const handleReply = async (comment: Comment) => {
    ...
    try {
      ...
      if (comment.platform === "instagram") {
        ...
        const igAccount = igAccountsStored.find(
          (a: any) => a.username === mediaItem?.accountUsername
        );
        ...
        const ytAccount = ytAccountsStored.find(
          (a: any) => a.name === mediaItem?.accountUsername
        );
  ...
  const totalComments = filteredMediaList.reduce(
          ...
          {selectedComments.size > 0 && (
            <div className="mt-4 flex items-center justify-between p-3 bg-pink-500/10 border border-pink-500/30 rounded-xl">
              <span className="text-sm text-pink-400 font-medium">
                {selectedComments.size} comments selected
              </span>
              <div className="flex items-center gap-2">
                <select
                  onChange={(e) =>
                    e.target.value && handleBulkReply(e.target.value)
                  }
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        {comment.authorProfileImage ? (
                          <img
                            src={
                              comment.authorProfileImage || "/placeholder.svg"
                            }
                          ...
                          <div className="space-y-2">
                            <div className="flex flex-wrap gap-2">
                              {quickReplies.map((reply, idx) => (
                                <button
                                  key={idx}
                                  onClick={() =>
                                    applyQuickReply(comment.id, reply)
                                  }
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                disabled={
                                  !replyText[comment.id] || replying[comment.id]
                                }
